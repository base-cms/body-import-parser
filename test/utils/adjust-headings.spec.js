...
describe('utils/adjust-headings', () => {
  it('should adjust heading elements when an <h1> is present.', async () => {
    ...
    expect($('h2').length).to.equal(0, 'Number of <h2> elements');
    expect($('h3').length).to.equal(1, 'Number of <h3> elements');
    expect($('h4').length).to.equal(2, 'Number of <h4> elements');
    expect($('h5').length).to.equal(2, 'Number of <h5> elements');
    expect($('h6').length).to.equal(3, 'Number of <h6> elements');
  });
  it('should adjust heading elements when an <h2> is present.', async () => {
    const body = `
      <div>
        <h2>Bar</h2>
        <h2>Bar</h2>
    ...
    expect($('h1').length).to.equal(0, 'Number of <h1> elements');
    expect($('h2').length).to.equal(0, 'Number of <h2> elements');
    expect($('h3').length).to.equal(0, 'Number of <h3> elements');
    expect($('h4').length).to.equal(2, 'Number of <h4> elements');
    expect($('h5').length).to.equal(2, 'Number of <h5> elements');
    expect($('h6').length).to.equal(3, 'Number of <h6> elements');
  });
